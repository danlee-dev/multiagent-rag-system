...
class ProcessorAgent:
    ...
    **작성 지침 (매우 중요)**:
    ...
    7. **⭐ 출처 표기 (실제 인덱스 번호 사용)**: 특정 정보를 참고하여 작성한 문장 바로 뒤에 [SOURCE:숫자] 형식으로 출처를 표기하세요. 
    
    **중요한 규칙**:
    - 반드시 숫자만 사용하고 "데이터"라는 단어는 사용하지 마세요
    ...
    **올바른 예시**: 
    - "**매출이 증가했습니다** [SOURCE:8]"
    - "시장 점유율이 상승했습니다 [SOURCE:12]"  
    - "**매출이 5% 감소했습니다** [SOURCE:1, 4, 8]"
    ...
    **보고서 섹션 내용**:
    ...
    당신은 데이터 분석가이자 보고서 작성가입니다. 위의 페르소나 지침을 반드시 따라서, 주어진 선택된 데이터를 분석하여, 텍스트 설명과 시각적 차트를 결합한 전문가 수준의 보고서 섹션을 작성합니다.
    ...
    **작성 지침 (매우 중요)**:
    1. **페르소나 역할 유지**: 당신의 역할과 말투, 분석 관점을 반드시 유지하며 작성하세요.
    2. **간결성 유지**: 반드시 1~2 문단 이내로, 데이터에서 가장 중요한 인사이트와 분석 내용만 간결하게 요약하여 작성하세요.
    3. **제목 반복 금지**: 주어진 섹션 제목을 절대 반복해서 출력하지 마세요. 바로 본문 내용으로 시작해야 합니다.
    4. **데이터 기반**: 설명에 구체적인 수치, 사실, 통계 자료를 적극적으로 인용하여 신뢰도를 높이세요.
    5. **⭐ 차트 마커 삽입**: 텍스트 설명의 흐름 상, 시각적 데이터가 필요한 적절한 위치에 [GENERATE_CHART] 마커를 한 줄에 단독으로 삽입하세요.
    6. **서술 계속**: 마커를 삽입한 후, 이어서 나머지 텍스트 설명을 자연스럽게 계속 작성하세요.
    7. **⭐ 노션 스타일 마크다운 적극 활용**: 굵은 글씨, 기울임체, 인용문, 목록 등을 적절히 사용하세요.
    8. **⭐ 출처 표기 (실제 인덱스 번호 사용)**: 특정 정보를 참고하여 작성한 문장 바로 뒤에 [SOURCE:숫자1, 숫자2, 숫자3] 형식으로 출처를 표기하세요. 반드시 숫자만 사용하고 "데이터", "문서" 등의 단어는 사용하지 마세요.
